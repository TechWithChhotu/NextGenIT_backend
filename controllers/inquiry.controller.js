...
// 1. Admin Template (Jo aapke paas aayega - Already built)
const adminHtmlTemplate = `...`; // Aapka existing template

// 2. Client Template (Jo user ko receive hoga brand positioning ke sath)
const clientHtmlTemplate = `
  <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden; background-color: #ffffff;">
    <div style="background-color: #020921; padding: 24px; text-align: center; border-bottom: 3px solid #2563eb;">
      <h2 style="color: #ffffff; margin: 0; font-size: 20px;">💼 Thank You for Connecting with Us</h2>
      <p style="color: #94a3b8; margin: 4px 0 0 0; font-size: 12px; font-family: monospace;">TechSolex — Enterprise Solutions</p>
    </div>
    ...
    <div style="padding: 24px; color: #0f172a; line-height: 1.6;">
      ...
      <p style="font-size: 14px; color: #475569;">We have successfully received your inquiry regarding <strong>${serviceInterest.toUpperCase()}</strong>. Our consulting team is reviewing your requirements and estimated budget ($${estimatedBudget}) to prepare a tailored roadmap for you.</p>
      
      <blockquote style="background-color: #f8fafc; border-left: 4px solid #2563eb; padding: 12px; margin: 20px 0; font-style: italic; color: #475569;">
        "Our team typically responds with an actionable proposal within 12-24 business hours."
      </blockquote>
      
      <p style="font-size: 14px; color: #475569;">If you want to add any immediate details, feel free to reply directly to this email.</p>
      
      <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 24px 0;" />
      
      <p style="font-size: 13px; margin-bottom: 0; color: #64748b;">Best Regards,<br><strong style="color: #020921;">TechSolex Operations Team</strong></p>
    </div>

    <div style="background-color: #f1f5f9; padding: 16px; text-align: center; border-top: 1px solid #e2e8f0;">
      <p style="margin: 0; font-size: 11px; color: #94a3b8;">&copy; 2026 TechSolex. All rights reserved.</p>
    </div>
  </div>
`;
...
const adminMailOptions = {
  ...
  replyTo: email,
  subject: `🚨 New Lead: ${name} - ${serviceInterest}`,
  html: htmlTemplate, // Aapka purana template
};

// Mail Options for Client
const clientMailOptions = {
  from: `"TechSolex" <${process.env.EMAIL_USER}>`,
  to: email, // Client ka email
  subject: `Confirmation: We've received your inquiry - TechSolex`,
  html: clientHtmlTemplate,
};

// Dono ko parallelly send karein
try {
  await Promise.all([
  ...
  console.log(
    "📨 Both Admin Alert & Client Acknowledgement Emails Dispatched Cleanly.",
  );
} catch (mailError) {
  console.error("❌ Nodemailer SMTP Error Failure:", mailError);
}
